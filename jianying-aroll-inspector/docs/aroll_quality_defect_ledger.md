...
```text
%USERPROFILE%\.auto_clip_runtime\aroll_v21_audits\quality_defect_ledger
```
...
```yaml
aroll:
  quality_defect_ledger_dir: "${HOME}/.auto_clip_runtime/aroll_v21_audits/quality_defect_ledger"
```

Environment override:

```powershell
$env:AUTO_CLIP_AROLL_QUALITY_DEFECT_LEDGER_DIR = "$env:USERPROFILE\.auto_clip_runtime\aroll_v21_audits\quality_defect_ledger"
```
...
py -3 tools\aroll_quality_defect_ledger.py `
  --run-dir "$env:USERPROFILE\.auto_clip_runtime\aroll_v21_uat_runs\<run>" `
...
py -3 tools\aroll_quality_defect_ledger.py `
  --run-dir "$env:USERPROFILE\.auto_clip_runtime\aroll_v21_uat_runs\<run>" `
  --issues-json "$env:USERPROFILE\.auto_clip_runtime\aroll_v21_audits\qc_issues.json" `
